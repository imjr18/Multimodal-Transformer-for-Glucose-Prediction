"""Mixed-precision training utilities for Part B models."""
...
def _autocast_context(enabled: bool):
    """Return the requested autocast context or a no-op context on CPU."""
...
def _run_training_epoch(
    ...
    *,
    ...
    for hr_sequence, glucose_context, ecg_features, emg_features, targets, _ in progress_bar:
...
def _run_validation_epoch(
    ...
    *,
    ...
    with torch.no_grad():
        for hr_sequence, glucose_context, ecg_features, emg_features, targets, _ in data_loader:
            hr_sequence = hr_sequence.to(device, non_blocking=True)
            glucose_context = glucose_context.to(device, non_blocking=True)
            ecg_features = ecg_features.to(device, non_blocking=True)
            emg_features = emg_features.to(device, non_blocking=True)
...
def train_multimodal_model(
    ...
    *,
    ...
    """Train a Part B model with mixed precision and validation early stopping."""
...
__all__ = ["train_multimodal_model"]
